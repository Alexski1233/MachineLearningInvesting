...
def backtest_top_n(test: pd.DataFrame, n: int = TOP_N) -> pd.DataFrame:
    ...
    """
    labeled = test[test[TARGET_COL].notna()]
    rebalance_dates = labeled["date"].drop_duplicates().sort_values().iloc[::REBALANCE_EVERY_DAYS]
    rows = []
    for d in rebalance_dates:
        ...
        rows.append(
            {
                "date": d,
                "ret": picks[TARGET_COL].mean(),
                "benchmark_ret": snap[TARGET_COL].mean(),
                "excess_ret": picks[TARGET_COL].mean() - snap[TARGET_COL].mean(),
                "hit_rate": (picks[TARGET_COL] > 0).mean(),
