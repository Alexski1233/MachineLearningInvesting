...
def print_holding_period_sweep(sweep) -> None:
    ...
    print_section("Holding-Period Robustness")
    if sweep.empty:
    ...
    for _, row in sweep.iterrows():
        rows.append((
            ...
            f"{int(row['periods'])}",
            format_pct(row["avg_ret"]),
            format_pct(row["avg_benchmark_ret"]),
            format_pct(row["avg_excess_ret"]),
            format_pct(row["total_return"], digits=1),
            format_pct(row["benchmark_total_return"], digits=1),
            format_pct(row["ear"], digits=1),
        ))
    fields = ["Hold", "Periods", "Avg ret", "EW avg ret", "Excess", "Total", "EW total", "EAR"]
    align = {"Hold": "r", "Periods": "r", "Avg ret": "r", "EW avg ret": "r", "Excess": "r", "Total": "r", "EW total": "r", "EAR": "r"}
    print_table(fields, rows, align=align)
